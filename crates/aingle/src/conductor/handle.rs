...
pub trait ConductorHandleT: Send + Sync {
    ...
    /// Get AI validation prediction for a transaction (read-only, for logging/observation)
    #[cfg(feature = "ai-integration")]
    ...
    /// Check if fast-path validation should be used for a prediction
    #[cfg(feature = "ai-integration")]
    fn ai_should_use_fast_path(&self, prediction: &aingle_ai::ValidationPrediction) -> bool;

    /// Record fast-path validation outcome for AI learning
    #[cfg(feature = "ai-integration")]
...
impl<DS: SafStore + 'static> ConductorHandleT for ConductorHandleImpl<DS> {
    ...
    #[cfg(feature = "ai-integration")]
    fn is_ai_enabled(&self) -> bool {
        // Use try_read to avoid blocking - if we can't get the lock, just return false
        self.conductor
            .try_read()
            .map(|lock| lock.is_ai_enabled())
            .unwrap_or(false)
    }
    ...
    async fn ai_predict_validation(
        ...
        let lock = self.conductor.read().await;
        if let Some(ai_service) = lock.ai_service() {
            let tx =
                super::ai_service::create_ai_transaction(hash, timestamp, agent, entry_type, data);
            let service = ai_service.read();
            service.predict_validation(&tx)
        } else {
            None
        }
    }
    ...
    fn ai_should_use_fast_path(&self, prediction: &aingle_ai::ValidationPrediction) -> bool {
        self.conductor
            ...
            .and_then(|lock| {
                lock.ai_service()
                    .map(|ai| ai.read().should_use_fast_path(prediction))
            })
            .unwrap_or(false)
    }
    ...
    #[cfg(feature = "ai-integration")]
    fn ai_record_fast_path_outcome(&self, was_valid: bool) {
        if let Ok(lock) = self.conductor.try_read() {
            if let Some(ai_service) = lock.ai_service() {
                ai_service.read().record_fast_path_outcome(was_valid);
            }
        }
    }
    ...
    async fn ai_determine_consensus_level(
        ...
        if let Some(ai_service) = lock.ai_service() {
            let tx =
                super::ai_service::create_ai_transaction(hash, timestamp, agent, entry_type, data);
            let service = ai_service.read();
            service.determine_consensus_level(&tx)
        } else {
            // Default to Full consensus when AI is not available
