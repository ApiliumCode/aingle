...
async fn sys_validation_workflow_inner(
    ...
    for so in sorted_ops {
        ...
        // AI Prediction (Phase 2/3: prediction and fast-path)
        #[cfg(feature = "ai-integration")]
        let (ai_prediction, use_fast_path) = {
            // Extract data from SgdOp for AI prediction
            let hash: [u8; 32] = op_hash.get_raw_32().try_into().unwrap_or([0u8; 32]);
            let header = op.header();
            let timestamp = header.timestamp().to_sql_ms_lossy() as u64;
            let agent: [u8; 32] = header.author().get_raw_32().try_into().unwrap_or([0u8; 32]);
            let entry_type = format!("{:?}", op.get_type());
            // Use signature bytes as additional data
            let data = op.signature().0.to_vec();

            let prediction = conductor_api
                .ai_predict_validation(hash, timestamp, agent, entry_type, data)
                .await;

            // Phase 3: Check if fast-path should be used
            let fast_path = prediction
                .as_ref()
                .map(|p| conductor_api.ai_should_use_fast_path(p))
                .unwrap_or(false);

            if fast_path {
                debug!(
                    op_hash = ?op_hash,
                    confidence = prediction.as_ref().map(|p| p.confidence),
                    "AI fast-path triggered - high confidence valid prediction"
                );
            }

            (prediction, fast_path)
        };

        #[cfg(not(feature = "ai-integration"))]
        let use_fast_path = false;
        ...
        // Note: Even with fast-path, we still validate for now.
        // In future optimization, fast-path could skip some expensive checks.
        ...
        // Log and learn from AI prediction vs actual outcome
        #[cfg(feature = "ai-integration")]
        {
            let actual_valid = matches!(outcome, Outcome::Accepted | Outcome::SkipAppValidation);

            if let Some(ref prediction) = ai_prediction {
                trace!(
                    op_hash = ?op_hash,
                    ...
                    "AI validation prediction vs actual"
                );
            }

            // Phase 3: Record fast-path outcome for learning
            if use_fast_path {
                conductor_api.ai_record_fast_path_outcome(actual_valid);

                if !actual_valid {
                    // Fast-path predicted valid but actual was invalid - log warning
                    warn!(
                        op_hash = ?op_hash,
                        "AI fast-path was incorrect - predicted valid but validation failed"
                    );
                }
            }
        }

        // Suppress unused variable warning when ai-integration is disabled
        let _ = use_fast_path;
