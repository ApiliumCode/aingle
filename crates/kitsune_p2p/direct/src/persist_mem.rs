...
struct PersistMemInner {
    ...
    priv_keys: HashMap<KdHash, sodoken::BufReadSized<{ sodoken::sign::SECRETKEYBYTES }>>,
...
impl AsKdPersist for PersistMem {
    ...
    fn generate_signing_keypair(&self) -> BoxFuture<'static, KdResult<KdHash>> {
        let inner = self.0.clone();
        async move {
            let pk = sodoken::BufWriteSized::<{ sodoken::sign::PUBLICKEYBYTES }>::new_no_lock();
            let sk = sodoken::BufWriteSized::<{ sodoken::sign::SECRETKEYBYTES }>::new_mem_locked()
                .map_err(KdError::other)?;
            ...
            let sk_read = sk.to_read_sized();
            inner
                .share_mut(move |i, _| {
                    i.priv_keys.insert(pk_hash_clone, sk_read);
                    Ok(())
                })
                .map_err(KdError::other)?;
    ...
    fn sign(&self, pub_key: KdHash, data: &[u8]) -> BoxFuture<'static, KdResult<Arc<[u8; 64]>>> {
        let data = sodoken::BufRead::from(data.to_vec());
        let sk = self
            .0
