                ...
                match network_type {
                    NetworkType::QuicMdns => {
                        ...
                        if !urls.is_empty() {
                            ...
                            let agent_b64 = URL_SAFE_NO_PAD.encode(&agent[..]);
...
impl KitsuneP2pHandler for Space {
                        ...
                        while let Some(maybe_response) = stream.next().await {
                            match maybe_response {
                                Ok(response) => {
                                    ...
                                    // Decode response
                                    let remote_agent_vec = URL_SAFE_NO_PAD
                                        .decode(&response.service_name[..])
    ...
    fn handle_notify_multi(
        ...
        let inner_fut = self.handle_notify_multi_inner(input)?;
