...
async fn tls_server(
    ...
    let res: TransportResult<()> = async {
        let mut srv =
            rustls::ServerConnection::new(tls_server_config).map_err(TransportError::other)?;
                ...
                Some(Left(Some(data))) => {
                    ...
                    while srv.wants_write() {
                        ...
                        srv.write_tls(&mut tls_data)
                            .map_err(TransportError::other)?;
                        if !tls_data.is_empty() {
                            tracing::trace!("{}: SRV tls flush {} bytes", short, tls_data.len());
                            write
                                .send(ProxyWire::chan_send(tls_data.into()))
                                .await
                                .map_err(TransportError::other)?;
                ...
                Some(Right(Some(wire))) => match wire {
                    ProxyWire::ChanSend(data) => {
                        ...
                        loop {
                            ...
                            while let Ok(size) = srv.reader().read(&mut buf) {
