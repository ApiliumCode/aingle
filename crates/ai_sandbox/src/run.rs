...
pub async fn run(
    ...
    let (port, mut aingle) = run_async(aingle_path, sandbox_path.clone(), force_admin_port).await?;
...
async fn check_started(aingle: &mut Child) {
    let started = tokio::time::timeout(std::time::Duration::from_millis(20), aingle.wait()).await;
