...
pub mod error;
mod saf_gamut;
use crate::{properties::YamlProperties, saf::SafBundle};
...
pub struct RegisterSafPayload {
    ...
    /// Note: This was previously `#[serde(flatten)]` but that's incompatible with
    /// msgpack's struct_map serialization mode used by aingle_middleware_bytes.
...
impl InstalledAppCommon {
    ...
    pub fn cloned_cells(&self) -> impl Iterator<Item = &CellId> {
        self.slots.values().flat_map(|slot| &slot.clones)
    }
